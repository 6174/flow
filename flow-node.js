...
var Node = function Node(dArg, iArg) {
    ...
    for (var ii in i) this.inputs[ii] = {
        src: {
            val: i[ii],
            flag: -1
        }
    }; // -1 used for one-off updates
    for (var ii in o) this.outputs[ii] = {
        node: this,
        val: o[ii],
        flag: false
    };
    ...
    //widget.upLoc();
    widget.resize();
    if (iArg.x && iArg.y) widget.upLoc(iArg.x, iArg.y);
    this.widget = widget;
    ...
    this.draw = function() {
        if (vChange) {
            if (dArg.draw) dArg.draw(i, o, this);
    ...
    this.eval = function() {
        ...
        for (var ii in i) {
            if (this.inputs[ii].src.flag) {
                ...
                if (this.inputs[ii].src.flag == -1) this.inputs[ii].src.flag = false;
                vChange = true;
            }
        }
        if (run) {
    ...
    this.remove = function() {
        if (dArg.remove) dArg.remove(i, o, this);
        //unlink inputs
