var Wire = function Wire(p1, p2, color) {
    ...
    this.redraw = function() {
        ...
        ctx.bezierCurveTo(half, p1[1] - y, half, p2[1] - y, p2[0] - x, p2[1] - y);
...
function mkWire(n1, p1, n2, p2, color) { // link p1 (output) to p2 (input)
    ...
    if (isFinite(color)) color = 'hsl(' + color + ', 50%, 55%)';
    // unlink existing?
    //link nodes
    ...
    if (!n1.widget.outWires[p1]) n1.widget.outWires[p1] = [];
